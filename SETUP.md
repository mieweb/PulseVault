...
cd pulsevault
cp .env.example .env
# Edit .env: Set HMAC_SECRET (generate with: openssl rand -base64 32)
...
**Note:** SSL certificates are auto-generated by `dev-setup.sh`. For production, use Let's Encrypt.
...
cd /opt/pulsevault
cp .env.example .env
nano .env  # Set: NODE_ENV=production, HMAC_SECRET, paths
```
...
- [ ] `./scripts/dev-setup.sh` completed
