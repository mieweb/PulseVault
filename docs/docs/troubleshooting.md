...
| 401/502 when external app calls /api/qr/deeplink | [External app → API key](#external-app-api-key-errors) |
...
## External app (API key) {#external-app-api-key-errors}

**When:** An external app (e.g. Time Harbour) calls PulseVault’s `POST` or `GET /api/qr/deeplink` and gets 401 or 502.

**401 — Missing or invalid API key:**

- Request must include the key in `X-API-Key: pv_...` or `Authorization: Bearer pv_...`.
- Key must be created in PulseVault Admin and not revoked or expired.
- Key is shown only once at creation; if lost, create a new key in Admin and revoke the old one.

**502 — Backend unreachable:**

- PulseVault frontend needs `BACKEND_URL` and `BETTER_AUTH_URL` set so it can call the backend for token generation. Check server env and that the backend is up.

See [External apps (API key)](./uploads/external-apps) for request format and error responses.
