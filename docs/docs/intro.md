...
│  or External App│  pulsecam://...    │  (Record &  │  TUS / Chunk  │  (PulseVault │
│  (API key)      │                    │   Edit)     │               │   or custom) │
└─────────────────┘                    └─────────────┘               └──────────────┘
```

1. **Your application (or an external app with an API key) generates a deep link** with a `draftId`, upload server URL, and auth token.
...
flowchart LR
    subgraph Your path
        A[Your Web App] -->|Session or your backend| B[Get deeplink]
        B --> C[Show QR / link]
    end
    subgraph External app path
        D[External App e.g. Time Harbour] -->|API key| E[PV Frontend /api/qr/deeplink]
        E --> F[Show QR / link]
    end
    C --> G[Pulse Cam]
    F --> G
    G --> H[PulseVault or your server]
```
...
|----------|----------|----------|
| [**PulseVault**](./uploads/pulsevault) | TUS + managed server | Turnkey — deploy and go; Nginx, Proxmox/LXC, session + API-key deeplink |
| [**External apps (API key)**](./uploads/external-apps) | Same as PulseVault | Another app (e.g. Time Harbour) shows “Record a short” per ticket; no shared login; “Uploaded via X” on dashboard |
...
| **PulseVault** | Reference upload server and web app (Docker: Nginx, Next.js frontend, Fastify backend); session + API-key deeplink, dashboard, admin. |
| **API key** | Server-to-server key for external apps to call PulseVault’s `/api/qr/deeplink` without user login; created in PV Admin. |

## Next steps

- **First time?** → [Launching Pulse Cam](./launching) (deep link format, QR workflow, testing).
- **Using PulseVault?** → [PulseVault](./uploads/pulsevault) (deploy, session vs API-key, Nginx/media security).
- **External app (e.g. Time Harbour)?** → [External apps (API key)](./uploads/external-apps) (API key, draftId, “Uploaded via X”).
